...
```
    ...
    [?25l     [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m0.0/7.8 MB[0m [31m?[0m eta [36m-:--:--[0m[2K     [91m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m[91m╸[0m [32m7.8/7.8 MB[0m [31m252.7 MB/s[0m eta [36m0:00:01[0m[2K     [90m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[0m [32m7.8/7.8 MB[0m [31m144.9 MB/s[0m eta [36m0:00:00[0m
    ...
    Requirement already satisfied: filelock in /usr/local/lib/python3.12/dist-packages (from torch>=1.7.1->grad-cam) (3.25.2)
    ...
    Requirement already satisfied: cuda-pathfinder~=1.1 in /usr/local/lib/python3.12/dist-packages (from cuda-bindings==12.9.4->torch>=1.7.1->grad-cam) (1.5.2)
    ...
    Requirement already satisfied: packaging>=20.0 in /usr/local/lib/python3.12/dist-packages (from matplotlib->grad-cam) (26.0)
    ...
    Building wheels for collected packages: grad-cam
      ...
      Created wheel for grad-cam: filename=grad_cam-1.5.5-py3-none-any.whl size=44286 sha256=05d55c94f1c455b2734fe50d057263fe0b8879425d3cfe52db86c9dfba953806
...
file_name = random_img_info['file_name']
img_path = os.path.join(NEW_IMG_DIR, file_name)

rgb_img_pil = Image.open(img_path).convert("RGB")

input_tensor = val_test_transform(rgb_img_pil).unsqueeze(0).to(device)

rgb_img_float = np.array(rgb_img_pil).astype(np.float32) / 255.0
```


```python
print(coco_data['images'][2])
```

    {'id': 3, 'width': 1600, 'height': 1200, 'file_name': 'avail_bckm_3.jpg', 'license': 0, 'flickr_url': '', 'coco_url': '', 'date_captured': 0}
...
if len(results['boxes']) > 0:
    target_box = results['boxes'][0].detach().cpu().numpy()
    target_label = results['labels'][0].detach().cpu().numpy()

    top_label = int(target_label)
    class_name = unh_classes.get(top_label, f"ID:{top_label}")

    targets = [FasterRCNNBoxScoreTarget(labels=[target_label], bounding_boxes=[target_box])]
    grayscale_cam = cam(input_tensor=input_tensor, targets=targets)
    grayscale_cam = grayscale_cam[0, :]

    h_tensor, w_tensor = input_tensor.shape[2], input_tensor.shape[3]
    rgb_img_normalized = np.array(rgb_img_pil.resize((w_tensor, h_tensor))).astype(np.float32) / 255.0

    visualization = show_cam_on_image(rgb_img_normalized, grayscale_cam, use_rgb=True)

    plt.figure(figsize=(12, 8))
    plt.imshow(visualization)
    plt.title(f"Gradient Weighted CAM : {class_name} (ID: {top_label})")
    plt.axis('off')
    plt.show()
...
```python
random_img_info = coco_data['images'][2] #image 2
...
```python
random_img_info = coco_data['images'][3] #image 3
...
```python
random_img_info = coco_data['images'][4] #image 4
...
```python
random_img_info = coco_data['images'][6] #image 6
